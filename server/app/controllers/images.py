...
def get_image_with_lock_else_throw(db: Session, user: models.User, id: uuid.UUID) -> models.ImageUpload:
    """Note: This locks the image row. Make sure to unlock by calling db.commit()/db.rollback()."""
    image = maybe_get_image_with_lock(db, user, id)
    if image is None:
        raise ValueError("Invalid image")
    return image


def maybe_get_image_with_lock(db: Session, user: models.User, id: uuid.UUID) -> Optional[models.ImageUpload]:
    """
    Note: This locks the image row. Make sure to unlock by calling db.commit()/db.rollback().
