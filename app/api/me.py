...
@router.post(
    "",
    response_model=schemas.user.UpdateProfileResponse,
...
@router.post(
    "/preferences",
    response_model=schemas.user.UserPrefs,
    response_model_exclude_none=True,
)
...
async def get_suggested_users(
    ...
    users = [
        schemas.user.SuggestedUserItem(user=user_map.get(user_id), num_mutual_friends=num_mutual_friends)
...
async def follow_many(
    ...
    followed_or_blocked_subquery = union_all(
        ...
        select(models.UserRelation.from_user_id).where(
